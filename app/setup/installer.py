from app.core.database import SessionLocal
from app.models import Role
from app.services.role_service import RoleService
from app.services.user_service import UserService
from app.services.branch_service import BranchService



roles = ["Administrador", "Dueño", "Supervisor", "Cajero"]

def install_system():

    with SessionLocal() as db:
        try:
            existing_role = db.query(Role).filter(Role.name == "Admin").first()
            if existing_role:
                print("Sistema ya instalado")
                return

            role_service = RoleService(db)
            user_service = UserService(db)
            branch_service = BranchService(db)

            for role in roles:
                role_service.create_role({"name": role})

            branch_service.create_branch({"name" : "ApuByte sac", "phone": "946566438", "email": "apubyte@hotmail.com"})
            db.flush()
            role_obj = role_service.get_role(1)
            branch_obj = branch_service.get_branch(1)
            role_id = role_obj.id  # Extraemos solo el entero
            branch_id = branch_obj.id

            user_service.create_user({"name":"rodrigo",
                                      "last_name" : "fernandez",
                                      "email":"medina-1997@hotmail.com",
                                      "password":"Rofernan1997",
                                      "dni": "71114855",
                                      "username": "rofernan",
                                      "phone": "946566438",
                                      "role_id" : role_id,
                                      "branch_id" : branch_id,
                                      })

            db.commit()
            print("Datos iniciales OK ")

        except Exception as e:
            db.rollback()
            raise ValueError(f"Error en carga de datos iniciales: {e}")
